...
各段用**双下划线** `__` 分隔；**目录层级不限**（V4 起支持多级目录）：
...
| 字段 | 必填 | 类型 | 说明 |
|---|---|---|---|
| `path` | 推荐(多级) | string[] | **多级目录的权威写法**：目录名链（大类 → … → 放文档的目录），如 `path: [Java, 集合, 类]` |
| `slugs` | 建议(配合 path) | string[] | 与 `path` 一一对应的目录 slug 链，如 `slugs: [java, collections, class]` |
...
匹配顺序（后端实现）：每一级都在其上级目录下匹配 slug 精确（忽略大小写）→ 名称精确 → 名称忽略空格与大小写 → 都不中则新建并标记"自动创建"。

> **重写已有文档时**：保持 `slugs` / `slug`（或 `category_slug` / `topic_slug` / `slug`）不变，系统会自动生成新版本、保留历史，并尽力把用户写的"个人见解"重新挂回未改动的段落。**改了 `slug` 就会变成一篇新文档，见解不会跟过来。**
