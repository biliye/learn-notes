...
> **V3（多用户）**：注册/登录、数据按用户隔离、管理员查看全部文档，已通过本地端到端验证（33 项接口检查 + 浏览器实测），待服务器升级。
