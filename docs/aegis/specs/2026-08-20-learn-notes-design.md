...
### D11：图片按内容哈希落盘进用户目录，Nginx 直接托管，不建表、不上对象存储

- **决策**：
  - 上传接口 `POST /api/uploads/image`（multipart，单图 ≤ 5 MB，白名单 `png/jpg/jpeg/gif/webp`）。
  - 保存路径 `${app.storageDir}/../uploads/u<ownerId>/YYYY/MM/<sha256前16位>.<ext>`，**同一用户内**同图重复上传直接返回已有路径（天然去重）。
  - 对外 URL 为 `/uploads/u<ownerId>/YYYY/MM/<hash>.<ext>`，由 **Nginx 直接托管**该目录（`storage` 卷同时以只读方式挂给 web 容器），不经过后端流式转发。
  - **两代路径并存**：V5（图片归属）之前的 `uploads/YYYY/MM/<hash>.<ext>` 是**全局共享**的（同一张图所有人共用一个文件），老 URL 继续可访问，由 `ImageMigrationService` 一次性把老图复制进各用户目录并改写正文/见解引用（见 §D11.1）。
  - **不建 `doc_image` 表**：md 正文里的 `/uploads/...` 路径即引用关系，需要时用正则从 `content_md`、`doc_annotation.block_snippet`、`doc_annotation.content_md` 提取。
- **理由**：内容哈希天然去重且幂等（重复上传不产生垃圾）；按用户分目录让"删一个人的图片不影响别人"成为结构保证（V5 起），也是删账号能安全清图的前提；Nginx 托管静态图性能最好、实现最少；不建表避免"正文引用"与"图片表"两个真相不一致。
- **被否**：存数据库 BLOB（备份膨胀、无法用 CDN）；接对象存储 / OSS（自用规模无必要，且引入密钥管理与外部依赖，违背"云端挂了要能恢复"的自持目标）；保留全局去重 + 新增图片归属表（省磁盘但要多一张表与引用计数 GC，且多人共有的违规图无法只对一人删除）。
- **代价**：会产生**孤儿图片**（文档删了图还在）；跨用户不再去重（同一张图两人上传各存一份，自用规模可忽略）。孤儿图片当前**接受**：只在迁移报告里统计数量，不做自动删除——自动删图的误删风险高于占用磁盘的代价。
- **图片校验**：必须校验真实文件头（magic number）而非只信扩展名；文件名一律由服务端按哈希生成，绝不使用客户端文件名（防路径穿越与脚本上传）。

#### D11.1：V5 图片归属（2026-09-10）与老图迁移

- **问题**：全局去重让文件没有归属，删违规内容时无法只删某人的图，删账号也不敢动磁盘（怕删掉别人共用的文件）。
- **决策**：新上传进 `u<ownerId>/` 目录（去重只在自己账号内生效）；删账号时整目录清本人图片，老式共享图另按"全站是否还有他人引用"判定；老图用 `POST /api/admin/maintenance/migrate-images` 一次性迁移（**先 `dryRun=true` 看报告**）。
- **迁移三条安全约定**：① 只复制不移动（老 URL 不裂）；② 先落文件后改正文；③ 图片块 URL 变化会让块 `hash8` 改变，迁移须在重挂前把挂在图片块上的见解锚点前移，否则见解会被误判为 STALE。
- **引用计数口径**：`doc.content_md` + `doc_annotation.block_snippet` + `doc_annotation.content_md` 三处都要算（V5 修掉了此前漏扫见解正文、可能误删他人图片的问题）。
