...
- **账号创建**：无自助注册（`POST /api/auth/register` 已移除，登录页只有登录框）。新账号由管理员登录后在「全部文档」页顶部的「+ 新建用户」里创建：填用户名、昵称、初始密码（≥8 位）、角色（USER/ADMIN），建号即自动建好该用户的 INBOX/未归类分类。初始密码请转告本人，让其登录后用「修改密码」自行更换。
- **账号管理**：用户卡片上对**普通用户**提供「重置密码」与「删除」。重置密码不需要旧密码（忘记口令时的兜底），但**不会吊销对方已签发的 JWT**——本人已登录的页面在 token 到期前（默认 12 小时）仍可用。删除账号会连带清掉该用户的全部文档、历史版本、批注、整棵分类树**与他自己的图片目录**，不可恢复，界面要求输入用户名二次确认。
- **管理员账号不可被改密/删除**：改密与删号只接受普通用户作为目标（`ADMIN` 目标一律 403），既避免管理员互相夺号，也不存在「把自己删了」的自锁死路。
- **删除违规文档**：「全部文档」列表每行有「删除」，管理员可直接删任意用户的文档（后端本就允许，V5 起补上入口），历史版本、见解与不再被引用的图片一并清理。
- **图片归属（V5）**：新上传落到 `storage/uploads/u<用户id>/YYYY/MM/`，去重只在同一账号内生效；删账号整目录清本人图片，删文档只清"全站再无引用"的图。**老的共享图不会被动**（老 URL 继续可访问）。存量老图用 `POST /api/admin/maintenance/migrate-images?dryRun=true` 先看报告（复制几份、几张被多人共用），确认后 `dryRun=false` 执行——**执行前务必先 `scripts/backup.sh`**，它会复制老图并改写正文/见解引用，属不可逆的批量正文变更（幂等，可重跑）。
- **数据隔离**：每个用户只能看到自己的分类与文档（搜索/导入/导出同样只作用于自己的数据）。
- **管理员**：初始账号（`APP_ADMIN_USERNAME`）角色为 `ADMIN`，侧栏多出「全部文档」入口，可查看所有用户的文档、用户列表并建号/改密/删号；升级存量部署时原账号自动转为管理员，历史数据全部归属到该账号。
