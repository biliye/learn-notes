...
![Harness Engineering：模型之外的运行壳](/uploads/2026/09/20d6309748d443fb.png)

[在新标签页打开交互版架构图 ↗](/diagrams/harness-engineering.html)

运行壳一图看全：Agent 运行时被 Gate 审批（人类亲自敲回车放行）、Enforce 机械约束、Isolate 沙箱隔离、Persist 检查点、Observe 链路追踪与 Linter + CI 交付门禁层层包住——安全可控、高可靠、环境隔离、可观测四类保障全部落在模型之外的系统层。交互版支持按"审批与门禁""检查点与恢复""可观测闭环"三个视图聚焦。
